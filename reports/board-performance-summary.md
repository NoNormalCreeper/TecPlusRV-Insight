...
- 通过 `BOARD_BENCHMARK_CASES="memset_bench stride_bench crc32_bench" make board-benchmark PORT=COM9 BOOTLOAD_BAUD=115200` 上传新增 workload；
...
- 板级入口：`BOARD_BENCHMARK_CASES="memset_bench stride_bench crc32_bench" make board-benchmark PORT=COM9 BOOTLOAD_BAUD=115200`
...
- `board-benchmark` 可以自动生成 `results.csv`、`summary.md` 和各 case 原始串口日志。
...
1. 在干净工作区重新运行 `BOARD_BENCHMARK_CASES="memset_bench stride_bench crc32_bench" make board-benchmark PORT=COM9 BOOTLOAD_BAUD=115200`，把新的 run id 作为正式板级证据。
