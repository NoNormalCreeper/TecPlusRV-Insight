...
run_case() {
    ...
    echo "看到最终 PASS 后，按 Enter 结束当前 monitor 并进入下一个 case。"
    ...
    if ! grep -q '^RESULT:' "$board_log"; then
        echo "FAIL: $name 串口日志缺少 RESULT：$board_log" >&2
        exit 1
    fi
    if ! grep -Eq '^PASS\r?$' "$board_log"; then
        echo "FAIL: $name 串口日志缺少 PASS：$board_log" >&2
        exit 1
    fi
...
run_selected_case "perf_mix" "firmware/apps/baremetal/benchmarks/perf_mix.c"
run_selected_case "system_bench" "firmware/apps/baremetal/benchmarks/system_bench.c"
run_selected_case "riscv_tests_median" "firmware/apps/baremetal/benchmarks/riscv_bench_median.c"
run_selected_case "riscv_tests_memcpy" "firmware/apps/baremetal/benchmarks/riscv_bench_memcpy.c"
run_selected_case "memset_bench" "firmware/apps/baremetal/benchmarks/memset_bench.c"
run_selected_case "stride_bench" "firmware/apps/baremetal/benchmarks/stride_bench.c"
run_selected_case "crc32_bench" "firmware/apps/baremetal/benchmarks/crc32_bench.c"
run_selected_case "sdram_sum_bench" "firmware/apps/baremetal/benchmarks/sdram_sum_bench.c"
