# 面向 TEC-PLUS 资源约束平台的可验证、可拓展 RISC-V SoC 设计与实现：系统软件运行、交互调试与指令扩展
...
| 进阶 | 集成内存和 I/O，构建完整系统 | 64 KiB BRAM、32 MiB data-only SDRAM、UART、GPIO、VGA、蜂鸣器、交通灯、LED、按钮和 timer MMIO |
| 进阶 & 项目扩展 | 构建能够支持小型系统软件和应用运行的完整计算机系统 | DarkRISCV machine-mode trap、machine timer IRQ、CSR 与 `mret`、canonical trap frame，并在此基础上接入 FreeRTOS port 和 cooperative GDB stub |
...
项目引入了开源 PicoRV32、DarkRISCV、FreeRTOS Kernel 和官方 `riscv-tests`。我们保留 CPU 核和 RTOS kernel 的上游主体，把平台差异集中放在 adapter、wrapper、port 和 target environment 中。这样升级或排查上游模块时，不需要同时修改 SoC 和应用层。官方测试也没有直接改写，而是通过项目自己的 target environment 接入两颗 CPU。

项目在开发和验证过程中使用了多种开源工具。固件由 GNU RISC-V GCC 与 Binutils 完成编译、链接、镜像转换和反汇编，GNU GDB（实际中还用了 xPack 的 RISC-V GDB）用于连接自研 GDB stub。RTL 仿真主要使用 Icarus Verilog，GNU Make 和 Python 负责统一组织构建、测试、性能实验与串口下载。Bad Apple 媒体资源由 Python 脚本配合 FFmpeg、ffprobe、NumPy、SciPy 生成和预览，Bootloader host 通过 pyserial 访问开发板串口。项目同时使用 Git 管理版本和协作记录。Xilinx ISE 负责最终综合与实现，但该工具属于专有软件，因此不列入开源工具。
