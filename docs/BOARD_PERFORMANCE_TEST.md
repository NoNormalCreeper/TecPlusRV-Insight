...
只运行部分 case 时使用 `BOARD_BENCHMARK_CASES`：
...
| `stride_bench` | 不同步长访问 | 比较固定事务数下的 BRAM / SDRAM 地址模式成本；当前不等价于 Cache 局部性测试 |
...
所有可上板 benchmark 源文件都位于 `firmware/apps/baremetal/benchmarks/`；
`firmware/tests/` 不作为 Bootloader 用户程序入口。runner 只有同时采集到 `RESULT:` 和
最终 `PASS` 才接受当前 case。
